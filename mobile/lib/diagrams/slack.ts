...
export const SLACK: Diagram = {
  ...
  nodes: [
    ...
    },
    {
      ...
      sub: "token bucket per app per workspace",
      kind: "service",
      x: 440,
      y: 150,
      w: 240,
    ...
    },
    {
      id: "gateway",
      label: "Gateway pool",
      sub: "workspace-sticky, 100k sockets/box",
      kind: "service",
      x: 40,
      y: 250,
      w: 280,
    ...
    },
    {
      id: "messages",
      label: "Message store",
      sub: "messages + thread_replies keyspace",
      kind: "database",
      x: 440,
      y: 280,
      w: 240,
    ...
    },
    {
      ...
      sub: "persist stream, dedupe (user, msg)",
      kind: "service",
      x: 40,
      y: 610,
      w: 280,
    ...
    {
      id: "bridge",
      label: "Shared-channel bridge",
      sub: "membership + naming, no messages",
      kind: "database",
      x: 440,
      y: 610,
      w: 240,
      detail: {
        what: "A row on the visiting workspace's shard carrying the home shard, local channel name, membership and local notification preferences for a channel homed in another organisation.",
        why: "It is the third and last deliberate puncture of the tenancy boundary, after the directory and global identity. The bridge holds no messages, so there is still exactly one authoritative copy of the log and it belongs to exactly one tenant.",
        numbers: ["cross-shard hop 20 to 80ms", "~1% of message volume", "budget under 1s"],
        breaks:
        ...
        choice: {
          ...
          decider:
            "Whether two logs may disagree about order. Replication needs a single ordering authority anyway, or concurrent posts, edits and deletes interleave differently with no basis for reconciling, and once you have that authority you have single-home with extra copies. Single-home costs 20 to 80ms on ~1% of volume.",
          flips:
  ...
  edges: [
    ...
    },
    {
      ...
      from: "gateway",
      to: "bridge",
      label: "channel homed elsewhere?",
      dashed: true,
      fromSide: "right",
      toSide: "left",
      detail: {
        what: "The lookup that decides whether a channel is local to this shard or a shared channel homed in another organisation's shard.",
        why: "It is checked on the gateway rather than deeper down because the answer changes where the write goes. Everything else in the shard assumes local, and this is the enumerated exception that says when that assumption does not hold.",
        numbers: ["3 punctures total: directory, identity, shared channels"],
        breaks:
          "An external member's profile has to be resolvable to viewers on the other side, so identity is scoped per channel membership, a third scoping rule that exists solely for this feature.",
      },
    },
    {
      id: "e16",
      from: "bridge",
      to: "messages",
      label: "cross-shard hop, 20-80ms",
      fromSide: "right",
      toSide: "right",
      offset: 150,
      detail: {
        what: "A read or write from the visiting side routed to the home shard's keyspace, which is the only place the shared channel's log is ever written.",
        why: "One authoritative log per channel is what keeps the tenancy invariant intact on both sides. The alternative, replicating into both shards, needs an ordering authority anyway and then you have single-home with extra copies to reconcile.",
        numbers: ["20 to 80ms cross-region", "~1% of message volume", "budget under 1s"],
        breaks:
          "If the two organisations sit in conflicting residency regimes, no single-home placement satisfies both, and that is a contract question wearing an architecture costume.",
      },
    },
    {
      id: "e17",
