...
export const NOTIFICATION_SYSTEM: Diagram = {
  ...
  edges: [
    ...
    },
    {
      ...
      toSide: "left",
      detail: {
        what: "Email attempts and the bounce and complaint outcomes that the webhook consumer feeds back in.",
        why: "Bounce rate is the metric that gets an entire sending account paused, so it has to be visible continuously rather than discovered when every send starts failing at once.",
        numbers: ["alert above 5% stale-token prune rate per day"],
        breaks:
          "A rising bounce rate that nobody prunes against ends in an account-level send pause, which is a failure of your own making and follows you to the standby provider.",
      },
    },
    {
      id: "e18",
      ...
      toSide: "left",
      detail: {
        what: "The provisional state after the provider accepts an SMS, held open until the terminal state arrives.",
        why: "Recording the accept as delivered would overstate what is known. The state machine keeps Delivered and Confirmed apart precisely so the difference between accepted and confirmed stays visible in the data rather than being flattened by the dashboard.",
        breaks:
          "A webhook that never arrives leaves the record provisional forever, so there has to be a timeout that resolves it rather than a state that waits indefinitely.",
      },
    },
    {
      id: "e19",
    ...
    },
    {
      ...
      toSide: "right",
      detail: {
        what: "A message whose attempt budget is exhausted moving out of the retry path and into its channel's dead-letter queue.",
        why: "SMS reaches this arrow after one failure where push and email take four attempts over roughly an hour, and the budget stops in both cases because the alternative is spending attempts forever on a notification that has already gone stale.",
        numbers: ["SMS: 1 attempt", "push and email: 4 at 30s, 5min, 1h"],
        breaks:
          "Everything that lands here needs a human, so DLQ growth is a page. Automatically replaying it turns a visible failure pattern back into an invisible one.",
      },
    },
  ],
};
