...
export const MARKET_DATA_INGEST: Diagram = {
  ...
  nodes: [
    {
      id: "edge-group",
      label: "Ingest edge: the socket must never block",
      kind: "zone",
      x: 24,
      y: 94,
      w: 672,
    ...
    },
    {
      ...
      w: 280,
      detail: {
        what: "The venues themselves, each publishing a tick firehose as two identical multicast copies on separate network paths, plus a per-feed monotonic sequence number on every message.",
        why: "Drawn explicitly because it sets every constraint downstream answers to: a different wire protocol per venue, an arrival rate you do not control, and a transport that will never tell you it lost a packet.",
        numbers: ["2M msg/s planning peak at open, close and on news", "top 10 symbols carry 30-40% of all traffic", "~24-40B per packed canonical tick"],
        breaks:
          "The venue's sequence number is arrival order at our NIC, not the order the markets actually moved, so two venues quoting the same instrument are ranked by path length and nothing in this pipeline can fix that.",
      },
    },
    {
      id: "order-gateway",
    ...
    {
      id: "venue-recovery",
      label: "Venue recovery service",
      sub: "retransmit + snapshot channel",
      kind: "external",
      x: 440,
      y: 220,
      w: 240,
      detail: {
        what: "The venue's own repair endpoints: a request-response service that resends a missing sequence range, and a separate channel publishing periodic full state per instrument labelled with the sequence it is current as of.",
        why: "It is the second and third rungs of the ladder and it belongs to somebody else, which is why both rungs are rate limited and capped. Retransmit covers a hole of tens of messages; only the snapshot recovers from an unbounded gap.",
        numbers: ["retransmit round trip 5-50 ms", "snapshot interval typically 1-30 s"],
        breaks:
          "Venues cap how much they will resend and how often you may ask, and a venue-wide burst means every participant asks at once, so the recovery service is slowest exactly when you need it most.",
      },
    },
    {
      id: "normaliser",
      label: "Normaliser",
      sub: "decode into pre-allocated structs",
      kind: "service",
      x: 40,
      y: 220,
      w: 280,
  ...
  edges: [
    ...
    },
    {
      ...
      animated: true,
      detail: {
        what: "The main fan-out: each event routed to the single shard owning its instrument, in the sequence order it was stamped with.",
        why: "Partitioning rather than balancing. Ordering only has to hold within a book, so the same instrument always landing on the same consumer is what turns one total order into N independent ordered streams with no coordination.",
        numbers: ["~10k instruments across the shard set", "200 msg/s per instrument average, heavily skewed"],
        breaks: "Rebalancing shards moves instruments between consumers, and an event in flight during the move can arrive at the new owner behind one the old owner already applied.",
      },
    },
    {
      id: "e15",
    ...
    },
    {
      ...
      from: "gap-arb",
      to: "shards",
      label: "mark instrument stale",
      dashed: true,
      fromSide: "right",
      toSide: "right",
      offset: 120,
      detail: {
        what: "A per-instrument stale flag raised while a gap is open on a feed carrying that instrument, and cleared when the hole is filled.",
        why: "A book you know is missing messages is worse than no book, because a consumer will trade on it. The flag has to be visible in the published stream, otherwise downstream silently inherits a book it cannot tell is wrong.",
        numbers: ["staleness is per instrument, never global across ~10k"],
        breaks: "Making staleness global for convenience blinds the other 9,999 instruments over a hole in one symbol, which is a far larger outage than the gap itself.",
      },
    },
    {
      id: "e17",
