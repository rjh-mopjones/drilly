  ...
  nodes: [
    ...
    },
    {
      ...
      detail: {
        ...
        numbers: ["1 partition key: author_id", "~12k events/s steady, ~60k/s burst"],
        breaks:
    ...
    },
    {
      ...
      detail: {
        ...
        numbers: [
          ...
          "lower priority, same pool of ~50 workers as live fan-out",
        ],
        breaks:
    ...
    },
    {
      ...
      detail: {
        ...
        choice: {
          ...
          decider:
            "What the reader sees on their next load. The pull set has to be right within this single follow request or a fresh follow of a celebrity produces nothing at all; the backfill is historic content and can be seconds late without anyone noticing. CDC would make both eventually consistent and would make the first case visibly wrong.",
          flips:
    ...
    // ---- FRAME: the hybrid itself ----
    {
      ...
      detail: {
        ...
        numbers: ["tl:{user_id} capped at 1000", "ar:{author_id} capped at 200", "0 recovery work: rebuilt lazily from source"],
        breaks:
    ...
    // ----------------------------------------------------------- read path
    {
      ...
      detail: {
        ...
        why: "Merge, rank and hydrate are three stages of one request, sharing the reader's context, the connection pools and the 200ms budget. Ranking is still the one stage that can fail independently: it has its own timeout and a fallback to chronological order that this service applies when the ranker misses, even though it deploys and scales with the other two rather than as a separate service.",
    ...
    },
    {
      ...
      detail: {
        ...
        numbers: [
          ...
          "500 posts handed to ranking",
        ],
        breaks:
          "A pull-heavy reader fills the 500-post budget from the pull side alone, squeezing out whatever their pushed timeline held — and that reader is the new user whose follows are almost all large accounts.",
        choice: {
    ...
    },
    {
      ...
      detail: {
        ...
        breaks:
          "A load whose pool is heavily unhydratable returns short, and the over-fetch factor is tuned rather than derived, so it occasionally guesses wrong.",
        choice: {
  ...
  edges: [
    ...
    },
    {
      ...
      detail: {
        ...
        numbers: ["1 partition key: author_id", "5x burst absorbed here, not by Redis"],
        breaks:
    ...
    },
    {
      ...
      detail: {
        ...
        why: "It is a cached, batch-written flag rather than a live count because counting a 100 million edge follower list per post is not affordable, and the staleness that buys is exactly what the dual-mode window covers.",
    ...
    },
    {
      ...
      detail: {
        ...
        numbers: ["3 consequences per follow: edge, pull-set entry, backfill job", "a signup can carry 500 of these at once"],
        breaks:
    ...
    },
    {
      ...
      detail: {
        ...
        why: "It is the reason the classification is a batch product at all: counting 100 billion edges is affordable once a day off the request path and affordable nowhere else, and writing the result back to this same store is what the live worker's read (via the other edge into this store) depends on.",
    ...
    },
    {
      ...
      detail: {
        ...
        breaks:
          "Each extra source adds ~20 posts to a fixed 500-post budget, which is why the cap exists and why lowering the threshold to 1,000 would put ~40 sources here and blow the merge budget.",
      },
    },
    {
      id: "e24",
    ...
    },
    {
      ...
      detail: {
        ...
        why: "It is bounded at 500 deliberately: ranking cost is linear in pool size and the first stage costs ~5ms per 500, so the pool cap and the pull-source cap are the same constraint seen from two ends.",
        numbers: ["500 posts, ~1ms of merge CPU", "~35ms of ranking budget"],
        breaks:
          "This is the call the feed service must be willing to abandon. On a ranker timeout it serves the same pool in chronological order, and the reader is not told.",
      },
    },
    {
      id: "e26",
    ...
    },
    {
      ...
      detail: {
        ...
        breaks:
          "If ranking times out the fallback is chronological order over the same pool, so the feed degrades in quality rather than going blank, and the reader is not told.",
      },
    },
  ],
};
