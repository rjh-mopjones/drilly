  ...
  nodes: [
    {
      ...
      detail: {
        ...
        why: "Every node in the fleet is interchangeable, so this box is a role assignment rather than a tier. Membership and token ownership are gossiped between peers, which is why losing any one of the three costs a quorum member and never costs availability.",
    ...
    },
    {
      id: "coordinator",
      label: "Coordinator",
      sub: "any node, no storage needed",
      kind: "compute",
      x: 40,
      y: 100,
      w: 280,
      detail: {
        what: "Whichever node received the request. It locates the key's replica set, fans the request out, and waits for enough responses to satisfy the quorum.",
        why: "Separating the coordinator role from the storage role is the whole reason there is no single point of failure. Clients talk to a nearby node while replicas stay wherever the partitioning scheme says they belong, so no failover step and no election pause sits on the request path.",
    ...
    },
    {
      ...
      sub: "hash partitioning, 128 nodes/region",
      kind: "store",
      x: 440,
      y: 100,
      w: 240,
      detail: {
        what: "The token map: hash the key, walk the ring, take the next N distinct nodes as the owners of that range.",
        why: "Placement has to be computable on every node from gossiped state rather than looked up, because a placement service would be exactly the single point of failure the leaderless design exists to remove. Adding or removing a node moves only the ranges adjacent to its tokens.",
        numbers: ["128 nodes per region", "~400GB per node at 50TB physical"],
        breaks:
          "A hot key is placed on N replicas and nothing more, so one celebrity key drives 100x traffic at three specific nodes and the ring has no way to spread it.",
        choice: {
    ...
    },
    {
      ...
      sub: "3h TTL, ~60GB per outage",
      kind: "store",
      x: 440,
      y: 200,
      w: 240,
      detail: {
        what: "A write parked on a healthy neighbour, tagged with the replica it was intended for, replayed when that owner returns.",
        why: "It bridges the gap between a short outage and a full repair scan, so a node rebooting does not cost a range its write availability. The subtlety worth volunteering is that a hint must not count toward W under a strict quorum, because the hint holder is not a replica of the key.",
    ...
    },
    {
      ...
      detail: {
        ...
        why: "Quorums stay inside a region so the WAN never lands on the request path. The cross-region copy exists for a whole-DC failure and for serving readers near it, and it is drawn outside the boundary because you get no acknowledgement from it before answering the client.",
    ...
    },
    {
      ...
      w: 240,
      detail: {
        what: "The coordinator asks one replica for the value and the others for a digest, and on a mismatch pulls full values from all N, picks the winner by version, and writes it back to whoever was behind.",
        why: "It is the cheapest convergence mechanism because it rides traffic that was happening anyway. Digests keep the common case to one 1.2KB value on the wire instead of three, and the writeback happens off the response path so it costs the client nothing.",
    ...
    },
    {
      ...
      w: 240,
      detail: {
        what: "Each replica builds a Merkle tree over its token ranges, leaves hashing small key ranges. Two replicas compare roots and recurse only into subtrees that disagree.",
        why: "It is the only mechanism with complete coverage, so it is what backstops the cold keys read repair never touches and the writes hinted handoff dropped at the TTL. Message cost is logarithmic; the disk cost is not, because building the tree reads every key in the range.",
    ...
    },
    {
      ...
      w: 240,
      detail: {
        what: "A delete writes a marker carrying a version that dominates what it replaces, and the marker survives gc_grace_seconds before it can be purged.",
    ...
    },
    {
      ...
      w: 280,
      detail: {
        what: "A per-key counter tuple keyed by client actor. If one vector dominates componentwise it is newer; if neither dominates the writes are concurrent and both versions are returned.",
        why: "The residue of leaderlessness is two clients writing the same key at once, and every quorum setting accepts both. Vectors are the honest answer: they report concurrency instead of silently discarding a write, which timestamps cannot do because NTP error and write interarrival overlap.",
  ...
  edges: [
    {
      ...
      detail: {
        ...
        why: "The quorum is a per-request argument rather than a cluster setting, which is what lets one deployment serve a strict caller and a sloppy one at the same time. The client picks whichever nearby node it likes, because none of them is special.",
    ...
    },
    {
      ...
      from: "coordinator",
      to: "ring",
      label: "hash(key) → N owners",
      dashed: true,
      fromSide: "right",
      toSide: "left",
      detail: {
        what: "A local computation, not a network call: hash the key, walk the gossiped token map, take the next N distinct nodes.",
        why: "Drawn as a control path because no data moves. Placement must be derivable on every node from gossiped state, since a lookup service would reintroduce exactly the single point of failure that going leaderless was meant to remove.",
        numbers: ["N=3 owners out of 128 nodes"],
        breaks:
          "A node with a stale view of the ring during a topology change sends the request to a node that no longer owns the range, which is what makes membership convergence a correctness concern and not just bookkeeping.",
      },
    },
    {
      id: "e3",
      ...
      animated: true,
      detail: {
        what: "The request handed to the quorum counter, which dispatches to all three replicas in parallel and starts counting responses.",
        why: "Sending to all N and waiting for W or R is deliberate: fanning to only W would mean a single slow replica turns into a timeout instead of being outrun by the other two. The extra request is the cheap part.",
        numbers: ["fan to N=3, wait for W=2 or R=2"],
        breaks:
          "Every op touches all three replicas, so 1M client ops/s is 3M replica operations/s, and that multiplier is what sizes the fleet at 128 nodes rather than 50.",
      },
    },
    {
      id: "e4",
      from: "quorum",
      to: "replicas",
      label: "W acks / R responses",
      animated: true,
      detail: {
        what: "Parallel writes carrying the value and its version vector, or a read consisting of one full value request and N-1 digest requests.",
        why: "This is the hop the p99 budget is spent on. Waiting for the second of three responses lands near a single replica's p90, which is 4 to 6ms on a store with a 1ms median, and the whole 10ms budget is built around that number.",
        numbers: ["R=1 lands near ~1 to 2ms, R=2 near ~4 to 6ms", "10ms p99 read, 30ms p99 write"],
        breaks:
    ...
    },
    {
      ...
      from: "hints",
      to: "replicas",
      label: "replay on rejoin, throttled",
      dashed: true,
      fromSide: "left",
      toSide: "right",
      detail: {
    ...
    },
    {
      ...
      from: "anti-entropy",
      to: "replicas",
      label: "resync diverged subranges",
      dashed: true,
      fromSide: "left",
      toSide: "right",
      offset: 80,
      detail: {
        what: "Streaming the key ranges under the disagreeing subtrees from a healthy replica to the one that is behind.",
    ...
    },
    {
      ...
      from: "tombstones",
      to: "anti-entropy",
      label: "gc_grace > repair interval",
      dashed: true,
      fromSide: "right",
      toSide: "right",
      offset: 60,
    ...
    },
    {
      ...
      detail: {
        what: "The stored record read back out: value, version vector, TTL header, or a set of concurrent siblings if more than one version survives.",
