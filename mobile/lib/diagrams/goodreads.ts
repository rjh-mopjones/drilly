  ...
  nodes: [
    ...
    },
    {
      ...
      detail: {
        ...
        breaks:
          "Single-key write hotspot on a prize winner or book club pick. Above ~500 writes/s the work is promoted to sharded counters keyed on (work_id, hash(user_id) % N) and summed on read, which costs N reads per render. Duplicate or dropped events also drift the stored value from the truth silently, so a nightly job recomputes every work touched in the last 24h and alerts on the count that disagrees.",
        choice: {
