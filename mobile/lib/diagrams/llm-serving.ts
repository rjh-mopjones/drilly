...
export const LLM_SERVING: Diagram = {
  ...
  nodes: [
    ...
    },
    {
      ...
      row: 1,
      detail: {
        what: "Terminates TLS and SSE, authenticates the tenant, checks the quota, tokenises the prompt and pushes token ids onto the queue.",
        why: "Everything that can be decided without a GPU should be decided here, because a request rejected at the edge costs nothing while a request admitted to a replica costs memory that other streams needed. It also owns the only place a client disconnect is visible.",
        numbers: ["tokenisation ~3ms", "429 with Retry-After rather than unbounded queueing"],
        breaks:
          "Undetected disconnects. If TCP close is not turned into an abort the scheduler applies on the next step, generation continues, blocks stay held and the customer is billed for tokens nobody read.",
        choice: {
    ...
    },
    {
      id: "quota",
      label: "Tenant quota",
      sub: "Redis buckets, tokens/min",
      kind: "database",
      col: 1,
      row: 1,
      detail: {
        what: "Per-tenant token buckets keyed quota:{tenant}:{minute}, metering both requests/min and tokens/min (see #1).",
        why: "Tokens are the unit of cost, so they have to be the unit of the limit. Requests/min lets one tenant send a handful of enormous prompts that consume a replica's entire pool while sitting comfortably inside their nominal quota.",
        numbers: ["a 100k-token prompt is ~32GB of KV", "eight of them fill a 452GB pool"],
        breaks:
          "Bucket state is per-region and replicates asynchronously, so a tenant can briefly overspend across regions during a failover. That is accepted; the alternative is a synchronous check on the hot path.",
        choice: {
          pick: "Token buckets in Redis, metered on tokens/min as well as requests/min",
          instead: "Request-rate limiting only, or per-tenant concurrency caps alone.",
          decider:
            "The spread of cost per request. A 500-token prompt and a 100k-token prompt are one request each but 160MB and 32GB of pool respectively, a 200x difference, so a request counter meters nothing that matters. Concurrency caps help but still ignore context length.",
          flips:
            "A single internal consumer with one workload shape, where request count and token count are proportional and the extra accounting is pure overhead.",
        },
      },
    },
    {
  ...
  edges: [
    ...
    },
    {
      ...
      from: "gateway",
      to: "quota",
      label: "tokens/min bucket",
      dashed: true,
      fromSide: "right",
      toSide: "left",
      detail: {
        what: "Debiting the tenant's token bucket by the prompt length before the request is allowed to queue.",
        why: "The check has to happen before enqueue, because a queued request has already committed the platform to a deadline and a queue slot. Metering tokens rather than requests is what makes the limit denominated in the resource that is actually scarce.",
        numbers: ["buckets on both tokens/min and requests/min"],
        breaks:
          "Debiting on the prompt alone under-counts, since output tokens cost roughly 2.7x a prompt token here. The ledger reconciles afterwards, so a tenant can overshoot within a minute.",
      },
    },
    {
      id: "e3",
