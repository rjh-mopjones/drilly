...
export const TIKTOK: Diagram = {
  ...
  nodes: [
    ...
    },
    {
      ...
      detail: {
        what: "The learned retrieval channel: 10 billion video vectors as product-quantised codes, sharded, with a coarse quantizer routing the user vector to the relevant cells. Vectors are computed offline by a two-tower video encoder over creator, hashtags, sound, sampled frames and audio, one pass per video at end of transcode, and inserted before the video is feed-eligible.",
        why: "Scoring 10 billion candidates with the ranker is more compute than exists, so the corpus has to be narrowed by arithmetic that costs almost nothing per candidate. This index is what makes serving cost independent of catalogue size. The video encoder consumes no engagement features on purpose, so a video with zero views already has plausible neighbours instead of a cold-start branch bolted on the side.",
        numbers: [
          "10B vectors at ~16B per PQ code = ~160GB",
          "top-1000 in ~5ms",
          "re-ranked to top-200 against full vectors",
          "~100M video embeddings inserted/day, feature-only",
        ],
        breaks:
          "Retrieval failures are silent. Anything this index does not surface is never scored, never served and never measured, because you only observe the quality of what you served. A feature-only embedding is also a prior, not a measurement: two videos with near-identical audio and visuals routinely differ 3x in completion because one has a better first two seconds.",
        choice: {
