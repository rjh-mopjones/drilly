...
export const WEB_CRAWLER: Diagram = {
  ...
  figures: {
    frontier: {
      title: "Two-tier frontier: front queues for value, back queues for politeness",
      nodes: [
        { id: "new-urls", label: "New URLs", kind: "queue", col: 0, row: 0 },
        {
          id: "front-queues",
          label: "Front queues",
          sub: "F=10 priority bands",
          kind: "queue",
          col: 0,
          row: 1,
          detail: {
            what: "A value-ordered structure of roughly ten priority bands, not a total order.",
            why: "Bands rather than a strict order because a change-rate estimate and a domain-authority score are each only worth one significant figure.",
          },
        },
        { id: "pick", label: "Pick weighted", sub: "by priority band", kind: "service", col: 0, row: 2 },
        {
          id: "back-queues",
          label: "Back queues",
          sub: "1 per host, B=3× threads",
          kind: "queue",
          col: 0,
          row: 3,
          detail: {
            what: "A per-host structure under the invariant that a host lives in exactly one queue.",
            why: "A worker only ever looks at back queues whose next-allowed time has passed, so politeness is a property of storage rather than a check made at read time.",
          },
        },
        { id: "min-heap", label: "Min-heap", sub: "next_fetch_time", kind: "database", col: 0, row: 4 },
        { id: "crawler", label: "Crawler thread", sub: "fetch + parse + extract", kind: "service", col: 0, row: 5 },
      ],
      edges: [
        { id: "e1", from: "new-urls", to: "front-queues", tier: "hot", step: 1, label: "enqueue by score" },
        { id: "e2", from: "front-queues", to: "pick", tier: "hot", step: 2, label: "priority router" },
        { id: "e3", from: "pick", to: "back-queues", tier: "hot", step: 3, label: "admit to host queue" },
        { id: "e4", from: "back-queues", to: "min-heap", tier: "hot", step: 4, label: "ordered by readiness" },
        { id: "e5", from: "min-heap", to: "crawler", tier: "hot", step: 5, label: "head ready" },
        { id: "e6", from: "crawler", to: "new-urls", tier: "hot", step: 6, label: "fetch, parse, extract", fromSide: "right", toSide: "right" },
      ],
    },
    lifecycle: {
      title: "URL lifecycle: lease, retry, and dead-letter",
      nodes: [
        { id: "discovered", label: "Discovered", kind: "database", col: 0, row: 0 },
        { id: "queued", label: "Queued", kind: "database", col: 0, row: 1 },
        { id: "fetching", label: "Fetching", sub: "60s lease window", kind: "database", col: 0, row: 2 },
        { id: "parsed", label: "Parsed", kind: "database", col: 0, row: 3 },
        {
          id: "archived",
          label: "Archived",
          kind: "database",
          ...
          row: 4,
          detail: {
            what: "The URL's page is committed to storage and the lease is acknowledged.",
            why: "Acknowledging only after the write means a crash anywhere before it simply lets the lease expire and the URL become visible again. It re-enters Queued once revisit_ts elapses.",
          },
        },
        { id: "skipped", label: "Skipped", kind: "database", col: 1, row: 0 },
        {
          id: "retry",
          label: "Retry",
          sub: "per-host cooldown, jittered",
          kind: "database",
          col: 1,
          row: 2,
          detail: {
            what: "A retryable failure, requeued against the host's own cooldown rather than the URL's.",
            why: "Retry state is keyed to the host so one failing host does not pull every worker back on an identical schedule.",
          },
        },
        { id: "dead-letter", label: "DeadLetter", kind: "database", col: 1, row: 3 },
      ],
      edges: [
        { id: "e1", from: "discovered", to: "queued", tier: "hot", step: 1, label: "passes Bloom + robots" },
        { id: "e2", from: "queued", to: "fetching", tier: "hot", step: 2, label: "worker picks up" },
        { id: "e3", from: "fetching", to: "parsed", tier: "hot", step: 3, label: "200 OK" },
        { id: "e4", from: "parsed", to: "archived", tier: "hot", step: 4, label: "stored in object store" },
        { id: "e5", from: "discovered", to: "skipped", tier: "data", label: "blocked: robots/dedup" },
        { id: "e6", from: "fetching", to: "retry", tier: "data", label: "5xx / timeout / 429" },
        { id: "e7", from: "fetching", to: "dead-letter", tier: "data", label: "4xx permanent" },
        { id: "e8", from: "retry", to: "dead-letter", tier: "data", label: "max attempts" },
        { id: "e9", from: "retry", to: "queued", tier: "data", label: "backoff elapsed" },
      ],
    },
  },
};
