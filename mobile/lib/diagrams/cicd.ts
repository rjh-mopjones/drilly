...
export const CICD: Diagram = {
  ...
  nodes: [
    ...
    },
    {
      ...
      sub: "spec at head SHA, expands the DAG",
      kind: "service",
      x: 40,
      y: 110,
      w: 280,
    ...
    },
    {
      ...
      sub: "constraint matching, fair queueing",
      kind: "service",
      x: 40,
      y: 220,
      w: 280,
    ...
    {
      id: "identity-broker",
      label: "Identity broker (OIDC)",
      sub: "15-min ref-scoped credentials",
      kind: "service",
      x: 440,
      y: 220,
      w: 240,
      detail: {
        what: "Exchanges a job's signed OIDC token asserting this repo, this ref, this workflow, this run for a narrowly scoped, short-lived cloud credential.",
        why: "CI runs arbitrary code and sits next to production. Long-lived secrets on a worker turn any malicious build step into a permanent credential theft, so the design goal is that a fully compromised build yields something that expires in 15 minutes and can only touch one repo's resources.",
        numbers: ["15-minute credential lifetime", "workers hold zero long-lived secrets"],
        breaks:
          "The trust policy is the whole boundary. A broker policy written with a loose ref pattern hands production credentials to any branch a contributor can create.",
        choice: {
          pick: "OIDC token exchange per job, scoped to repo and ref",
          instead: "Long-lived secrets injected into the worker environment from a vault.",
          decider:
            "Blast radius of one compromised build step. A stored secret is valid until someone notices and rotates it, which is measured in weeks; an exchanged credential is valid for 15 minutes and only against one repo's resources. Fork PRs get a credential-free pool entirely, because they are code from strangers.",
          flips:
            "A closed farm building only first-party code with no external contributors, where the added identity plumbing buys little and the operational simplicity of a vault read is worth more.",
        },
      },
    },
    {
      ...
      detail: {
        ...
        why: "A VM's isolation boundary at roughly a container's start-up cost. A container shares the host kernel, and this fleet executes code written by whoever opened the pull request, so the kernel is the wrong trust boundary no matter how good the seccomp profile is.",
        numbers: ["~800ms snapshot resume", "~3,250 workers at peak, 4 vCPU each", "~10% (~330) kept pre-booted warm"],
        breaks:
          "The warm pool is what the queue-wait SLO actually rests on: at a merge wave it depletes, and observed p95 wait becomes cold-boot time rather than queueing.",
        choice: {
    ...
    },
    {
      ...
      sub: "signed, immutable, pinned by digest",
      kind: "database",
      x: 40,
      y: 460,
      w: 280,
    ...
    },
    {
      ...
      sub: "1% to 5% to 25% to cell to region",
      kind: "service",
      x: 40,
      y: 700,
      w: 280,
  ...
  edges: [
    ...
    },
    {
      ...
      from: "worker-pool",
      to: "identity-broker",
      label: "15-min scoped creds",
      dashed: true,
      fromSide: "right",
      toSide: "left",
      detail: {
        what: "The job presents its signed OIDC token and receives a short-lived credential scoped read-only to the cache and to that repo's resources.",
        why: "It exists so that nothing durable ever sits on a worker. The exchange happens per job rather than per pool, which is what lets a fork PR run on a pool that can obtain nothing at all.",
        numbers: ["15-minute lifetime", "read-only to the cache"],
        breaks:
          "The scope in the broker's trust policy is the entire security boundary; a loose ref pattern hands the credential to any branch a contributor can push.",
      },
    },
    {
      id: "e6",
