...
export const STRAVA: Diagram = {
  ...
  nodes: [
    ...
    {
      id: "dem",
      label: "Terrain raster (DEM)",
      sub: "30m tiles, memory-mapped",
      kind: "database",
      x: 440,
      y: 380,
      w: 250,
      detail: {
        what: "A global 30m-resolution grid of ground height, Copernicus or SRTM class, memory-mapped and sampled at each cleaned lat/lng.",
        why: "Elevation gain is a headline number athletes compare year over year, and the device cannot produce it. Sampling ground truth at each position and accumulating only monotone climbs above a ~3m threshold gives a figure that is stable across re-uploads of the same route.",
        numbers: ["30m resolution", "~3m monotone climb threshold", "612m gain on a 40km ride"],
        breaks:
          "A missing tile shard would otherwise process activities as zero climb, so the stage fails closed: the activity parks in a retry topic and the feed entry publishes without elevation rather than with a wrong number.",
        choice: {
          pick: "Sample a 30m terrain raster at each cleaned point",
          instead: "Sum positive deltas of the device's GPS or barometric altitude channel.",
          decider:
            "Noise accumulation. GPS altitude is noisy to about 15m, and summing that over 5,400 samples fabricates thousands of metres of climb on a flat ride, which is the single most reported wrong number in fitness tracking.",
          flips:
            "Indoor or off-grid activities with no usable raster coverage, and well-calibrated barometric altimeters, which are genuinely accurate for relative climb once drift-corrected against known ground height.",
        },
      },
    },
    {
  ...
  edges: [
    ...
    },
    {
      ...
      from: "workers",
      to: "dem",
      label: "sample per cleaned point",
      fromSide: "right",
      toSide: "left",
      detail: {
        what: "A ground-height lookup at every cleaned lat/lng, accumulating only monotone climbs above ~3m.",
        why: "Elevation is derived from the terrain rather than from the device because summed altitude noise is the single largest source of obviously wrong numbers, and a raster sample is deterministic so a reprocess reproduces the same total.",
        numbers: ["30m raster", "5,400 samples on a 90-minute ride"],
        breaks:
          "The raster version has to be stamped on the activity, because upgrading tiles silently changes historical climb totals and athletes compare those year over year.",
      },
    },
    {
      id: "e7",
