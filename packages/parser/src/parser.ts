...
export const SOURCES: Record<SourceId, SourceConfig> = {
  patterns: {
    ...
    // Transitional order while patterns.md migrates from the 16-section shape
    // to the 14-section one. Old and new names are interleaved into the same
    // slots so half-migrated questions still read top to bottom. Collapse to
    // just the 14 new names once every question is migrated.
    sectionOrder: [
      "Problem",
      "Core",
      "Summary",
